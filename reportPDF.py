...
def UpdateReportPdf():
    ...
    #leggi dati relativi agli strati di isolamento utilizzati
    with open('files/insulDataProg.csv') as file_insulation:
        reader_insulation = csv.reader(file_insulation)
        dataInsul = [row for row in reader_insulation]
    ...
    #pdf.create_table(Insulation data)
    pdf.create_table(table_data = dataInsul, title='Insulation Layers Data', cell_width='uneven')
    pdf.ln()
    pdf.ln()
