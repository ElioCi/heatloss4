...
def save_chart_as_image(chart, filename='files/grafico.png'):
    ...
    options.add_argument("--headless")  # Esempio: esegui in modalità headless
    options.add_argument("--disable-gpu")
    
    #driver = webdriver.Chrome(ChromeDriverManager().install(), options=options)
    service = Service(ChromeDriverManager().install())  # Usa Service per il driver
    driver = webdriver.Chrome(service=service, options=options)  # Passa options correttamente
...
def Esegui():
    ...
    with open('files/Output.csv') as file_out:
        ...
        if available_columns.issubset(df.columns):
            ...
            #save(combined_chart, "files/grafico.png", method="kaleido")
